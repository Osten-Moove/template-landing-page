import { ThemeMode, ThemeType } from '@/styles/Themes'
...
function ThemeStore({ children }: IThemeStore) {
  const [theme, setTheme] = useState<{ type: ThemeType; mode: ThemeMode }>({
    type: 'default',
    mode: 'light',
  })

  const switchTheme = (type: ThemeType, mode: ThemeMode = 'light') =>
    setTheme({ type, mode })
